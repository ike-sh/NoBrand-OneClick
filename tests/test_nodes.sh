...
assert_contains "$output" '已停止' 'stopped state text'
...
assert_contains "$status" '运行中: 1/1' 'Mieru aggregate status'
assert_contains "$status" '运行中: 1/2' 'Snell aggregate status'
assert_contains "$status" '运行中: 运行中' 'HY2 aggregate status'
assert_contains "$status" 'VLESS/Sudoku' 'VLESS aggregate status'
assert_contains "$status" '端口 / Port: 3606' 'VLESS aggregate port'
assert_contains "$status" '实例数 / Instances: 1' 'REALITY aggregate instance count'
