...
for source_file in src/*.sh scripts/*.sh tests/*.sh install-nobrand.sh install-mita.sh dist/install-nobrand.sh dist/install-mita.sh; do
  bash -n "$source_file"
done
if command -v shellcheck >/dev/null 2>&1; then
  shellcheck -S warning -x scripts/shellcheck-src.sh
  shellcheck -S warning install-nobrand.sh
fi
...
unit_tests=(
  ...
  tests/test_snell_v6_removed.sh
  tests/test_nodes.sh
