...
ROOT_ARTIFACT="${ROOT}/install-mita.sh"
DIST_DIR="${ROOT}/dist"
DIST_ARTIFACT="${DIST_DIR}/install-mita.sh"
...
tmp="$(mktemp "${ROOT}/.install-mita.build.XXXXXX")"
dist_tmp=""
cleanup() {
  [ -z "$tmp" ] || rm -f "$tmp"
...
if [ "$MODE" = check ]; then
  if [ ! -f "$ROOT_ARTIFACT" ] || ! cmp -s "$tmp" "$ROOT_ARTIFACT"; then
    printf 'build: root install-mita.sh is stale; run scripts/build.sh\n' >&2
    exit 1
  fi
  if [ ! -f "$DIST_ARTIFACT" ] || ! cmp -s "$tmp" "$DIST_ARTIFACT"; then
    printf 'build: dist/install-mita.sh is stale; run scripts/build.sh\n' >&2
    exit 1
  fi
...
mkdir -p "$DIST_DIR"
dist_tmp="$(mktemp "${DIST_DIR}/.install-mita.build.XXXXXX")"
