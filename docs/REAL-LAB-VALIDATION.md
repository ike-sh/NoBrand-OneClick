...
- 最终 IPLC 仍保留六个有效节点且全部服务 Running；`16800` 未被代理占用，公网 SSH 可持续重连。

## Lab contract

- 服务端：dev-ssh alias `IPLC`
- 客户端：dev-ssh alias `debian`
- 强制公网入口：`211.136.162.185`
- 代理 listener 允许范围：`16801-16899`
- `16800/TCP`：公网 SSH；TCP/UDP 数字 16800 均禁止分配给代理
- 不允许以 guest/private IP、localhost、hostname 或其它公网 IP 替代强制入口

实验室值没有进入 `src/`；只出现在本报告、被 `.gitignore` 排除的临时 `.lab/` 与远程 `/root/nobrand-lab/`。

## Baseline protection

- 权威 baseline：当前未提交 working tree，而不是旧 HEAD 的单协议内容。
- 开始开发前记录了 `git status --short`、`git diff --stat`、完整 diff 与最近提交。
- 本地安全快照：`.codex-backup/20260827-190755/`。
- 基线 HEAD：`2b3e2371746a0dd0248887a216d3a45d6ac8e95c`。
- 未使用 `reset --hard`、`clean`、全树 restore/checkout；未 commit、push、PR 或 Release。
- `D:\code\Xray-OneClick` 与 `D:\code\dev-ssh` 全程只读。
...
hostname: nbnet-1321
OS: Debian 13
guest IPv4: 172.16.1.168
default gateway: 172.16.1.1
guest sshd: 22/TCP
public SSH entry: 211.136.162.185:16800
```

公网 `16800 -> guest 22` 的 DNAT 不会出现在 guest `ss` 中。NoBrand 因此新增通用规则：从默认路由 IPv4 尾号推导出的 `xx00` 是保留基数，自动代理仍只从 `xx01-xx99` 选择；冲突 owner 显示为 `common:tail-base:<guest-ip>`。产品代码没有硬编码本实验室 IP 或 16800。

清理前发现的旧代理内容：

- `snell-server.service`
- `xray.service`
- host-network `remnanode` container（`remnawave/node:latest`，曾占用 TCP/16899）
- Remnawave 专属 nft tables `table ip remnanode` 与 `table ip6 remnanode6`

证据与可恢复备份：

```text
inventory: /root/nobrand-lab/preclean-20260827T121728Z
backup: /root/nobrand-lab/preclean-backup-20260827T121728Z.tar.gz
mode: 0600
SHA-256: 6e007d11df208e94f43987a9eb93bc987cda2b11f377ac8590c44ab1a4822ee2
```

只清理了已识别的旧代理 unit/path/container/table；没有 flush 全局 firewall。清理后重新建立公网 SSH 会话，确认 guest 22、默认路由、主机身份与网络均正常。

## Port validation

| Check | Result | Detail |
|---|---|---|
| derived block | PASS | `PORT_BASE=16800` |
| formal allocator sample | PASS | 120/120 位于 16801-16899；0 次选择 16800 |
| transport awareness | PASS | TCP/16898 占用时 UDP/16898 可用；反向也可用 |
| explicit TCP/16800 rejection | PASS | rc=1，无 Snell state/config/service 落盘 |
| SSH reconnect after rejection | PASS | 公网 211.136.162.185:16800 |

一次 400-sample 探针超过 SSH wrapper 的单次时间窗口，因没有完整输出而没有计为 PASS；随后正式重跑的 120 samples 位于规范要求的 100-500 范围内并完整 PASS。

第一次显式 16800 测试用“所有 Common 文件数完全不变”断言，被首次创建空 Common layout 影响。协议 state/config/service 均未产生；第二次将断言收窄到协议边界与 Common state hash 后 PASS。这是测试断言修正，不是 Snell state 泄漏。
...
|---|---|---|---|---|---|---:|---:|---|
| Mieru | 3.35.0 | TCP | TCP/16842 | `211.136.162.185:16842` | Mieru 3.35.0 | 20/20 | 5/5 | PASS |
| Snell v4 | 4.1.1 | TCP | TCP/16872 | `211.136.162.185:16872` | sing-box 1.14.0-rc.1 | 20/20 | 5/5 | PASS |
| Snell v5 | 5.0.1 | TCP | TCP/16812 | `211.136.162.185:16812` | sing-box 1.14.0-rc.1 (`version=4` wire-compatible mode) | 20/20 | 5/5 | PASS |
| Snell v6 | 6.0.0rc2 | TCP | TCP/16862 | `211.136.162.185:16862` | sing-box 1.14.0-rc.1 | 20/20 | 5/5 | PASS |
| Hysteria2 | Xray 26.3.27 | UDP | UDP/16810 | `211.136.162.185:16810` | sing-box 1.14.0-rc.1 | 20/20 | 5/5 | PASS |
| VLESS Sudoku | Xray 26.3.27 | TCP | TCP/16835 | `211.136.162.185:16835` | Xray 26.3.27 | 20/20 | 5/5 | PASS |
...
```text
IPLC: /root/nobrand-lab/client-bundle.tar.gz
local ignored lab: .lab/client-bundle.tar.gz
Debian: /root/nobrand-lab/client-bundle.tar.gz
SHA-256: 0b1669f4ef832ce37a16be1268f224effb03160d0fce42ea466300a274904bc2
mode: 0600
```

三段传输 SHA-256 一致。每个 remote target 都再次断言为 `211.136.162.185`；VLESS server/state/generated client/transferred client 再次通过 Encryption absence 与 Xray `run -test`。

完整最终 credential handoff：

```text
/root/nobrand-lab/final-nodes.txt
mode: 0600
```
...
Debian 原有 `127.0.0.1:1080` OpenSnell 与 `127.0.0.1:17880` network-lab listener 在测试前后 PID/listener 一致。NoBrand 只临时使用 18081-18086，同时启动六个 client；结束时只终止记录的测试 PID。

v6 公网异常出现后，先在 16862 重启服务，再由 Common Port 重装到 16895；两端 exporter/state/PSK 与官方 runtime 均一致。端口变更没有改变公网 payload blackhole，因此最终保留新端口 16895，并执行两次有界完整共存复测，不无限重试“刷”PASS。

| Protocol | Final listener | Final Display Endpoint | Requests round 1 | Requests round 2 | Public packets | Result |
|---|---|---|---:|---:|---:|---|
| Mieru | TCP/16842 | `211.136.162.185:16842` | 5/5 | 5/5 | 107 / 111 | PASS |
| Snell v4 | TCP/16872 | `211.136.162.185:16872` | 5/5 | 5/5 | 69 / 70 | PASS |
| Snell v5 | TCP/16812 | `211.136.162.185:16812` | 5/5 | 5/5 | 70 / 71 | PASS |
| Snell v6 | TCP/16895 | `211.136.162.185:16895` | 1/5 | 1/5 | 60 / 60 | ENVIRONMENT BLOCKED |
| Hysteria2 | UDP/16808 | `211.136.162.185:16808` | 5/5 | 5/5 | 61 / 66 | PASS |
| VLESS Sudoku | TCP/16805 | `211.136.162.185:16805` | 5/5 | 5/5 | 16 / 43 | PASS |

Snell v6 的双端证据：

1. Debian 到 `211.136.162.185:<v6-port>` 三次握手 PASS。
...
4. 同一官方 1.14.0-rc.1 sing-box、同一 PSK、同一 exporter outbound，通过 IPLC `127.0.0.1:<v6-port>` 回环访问 Cloudflare：PASS。
5. 公网 Snell v4 同期 3/3，最终两轮 v4/v5 仍 5/5；不是全局公网或 Debian 故障。
6. v6 从 16862 改到 16895 后相同，排除单一端口编号。
...
以下均 PASS，输出保存到 `/root/nobrand-lab/final-audit/` 的 `0600` 文件：
...
|---|---|---|
| public SSH DNAT | public TCP/16800 -> guest TCP/22 | PASS / reconnect PASS |
| Mieru | TCP/16842 | Running |
| Snell v4 | TCP/16872 | Running |
| Snell v5 | TCP/16812（官方 runtime 同时有同号 UDP relay listener） | Running |
| Snell v6 | TCP/16895 | Running |
| Hysteria2 | UDP/16808 | Running |
| VLESS Sudoku | TCP/16805 | Running |

所有代理端口位于 16801-16899；`PROXY_16800_COUNT=0`。所有 Display Host 为强制公网入口。
...
|---|---|---|---|---|---|---|
| NB-PORT-001 | High | guest 看不到 public 16800 DNAT，allocator 可能认为同号内部端口空闲 | NAT 外部端口不出现在 guest socket table | `src/16-core-port.sh`, `src/35-users-state.sh` | 通用保留默认路由 IPv4 尾号块的 `xx00`，新增 Common Port regression | explicit 16800 rejection PASS；120 samples 0 次选择 16800；SSH PASS |
...
| LAB-PORT-001 | Test | 首次 16800“文件总数不变”断言失败 | 首次运行只初始化空 Common layout | lab boundary assertion | 收窄为协议 state/config/service 与 Common hash | 第二次 PASS，无 Snell state 落盘 |
| LAB-TTY-001 | Test | TTY harness 两次停在真实 prompt | nested invalid input 有 pause；Mieru reconfigure 首 prompt 与初版 regex 不同 | ignored `.lab/tty_menu_test.py` | 按真实 UI 顺序匹配后整轮重跑 | 全菜单 TRUE_TTY PASS |
...
通过新的公网 SSH 会话复验 `211.136.162.185:16800`，连接 PASS。最终 `nobrand status`/`nodes`/`doctor` 与 `ss` 结果：
...
- Snell v5 的同进程 `UDP/16812` 只显示 auxiliary runtime INFO；canonical ownership 仍为 `TCP/16812`。
- Snell v6 control plane PASS，并显示 Experimental/environment-dependent INFO；普通 Doctor 没有公网联网 gate。
- `PROXY_16800_COUNT=0`；代理监听仍是 Mieru TCP/16842、Snell v4 TCP/16872、v5 TCP/16812、v6 TCP/16895、HY2 UDP/16808、VLESS TCP/16805。
...
- release 文件集 73 个；`.lab/` 与 `.codex-backup/` 均被 Git 忽略。
...
- 生产 `src/`、`scripts/` 没有真实实验 IP、16800/16801 或 `dev-ssh` 路径；实验 IP 只保留在本历史验证文档。
