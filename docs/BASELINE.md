...
原 `tests/run_all.sh` 的模块、syntax、ShellCheck、config、regressions、HY2 删除回滚、endpoint/security/binary rollback 通过。环境没有真实 Xray，明确输出 `[SKIP] 未找到可执行 xray，跳过 xray run -test`；该项不是 PASS。

NoBrand 后续单独下载官方 Xray 执行真实 `run -test`，并真实启动 Surge Snell v4/v5/v6 localhost listener。

## 第二阶段改动前保护与 baseline

- 权威第一阶段 HEAD：`2b3e2371746a0dd0248887a216d3a45d6ac8e95c`
- 未提交工作树完整快照：`.codex-backup/20260827-190755/`（被 `.gitignore` 排除）
...
禁止用 reset/clean/restore 覆盖这份 working-tree baseline。Xray-OneClick 与 dev-ssh 均保持只读。
